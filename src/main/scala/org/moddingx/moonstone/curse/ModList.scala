package org.moddingx.moonstone.curse
  ...
  def updateFileList(action: => Unit): Unit = {
    component.rebuild {
      ...
      for (projectId <- dependencies.keySet) {
        allFiles.get(projectId).orElse(fileIds.get(projectId).map(fileId => FileEntry(projectId, fileId, COMMON, locked = false))) match {
